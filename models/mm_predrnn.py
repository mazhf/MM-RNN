...
from models.ms_predrnn import PredRNN_cell
from util.attention import MAM


class Multi_Modal_PredRNN(nn.Module):
    def __init__(self, input_channel, output_channel, b_h_w, kernel_size, stride, padding):
        ...
        lstm_ele = [PredRNN_cell(input_channel, output_channel, [B, H, W], kernel_size, stride, padding),
                    PredRNN_cell(input_channel, output_channel, [B, H // 2, W // 2], kernel_size, stride, padding),
                    PredRNN_cell(input_channel, output_channel, [B, H // 4, W // 4], kernel_size, stride, padding),
                    PredRNN_cell(input_channel, output_channel, [B, H // 4, W // 4], kernel_size, stride, padding),
                    PredRNN_cell(input_channel, output_channel, [B, H // 2, W // 2], kernel_size, stride, padding),
    ...
    def forward(self, x_all, m_all, layer_hiddens, embed, embed_ele, fc, fc_ele):  # b8hw
        ...
        for l in range(self.n_layers):
            ...
            # elements
            x_ele, m_ele, next_hiddens_ele = self.lstm_ele[l](x_ele, m_ele, hiddens_ele)
