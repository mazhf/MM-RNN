...
class TrajGRU_cell(nn.Module):
    def __init__(self, input_channel, output_channel, b_h_w, kernel_size, stride, padding):
        ...
        # x to z r h_
        self._conv_x2h = cfg.LSTM_conv(in_channels=input_channel, out_channels=output_channel * 3, kernel_size=kernel_size, stride=stride, padding=padding)

        # inputs and hidden to flow
        self._conv_x2f = cfg.LSTM_conv(in_channels=input_channel, out_channels=output_channel, kernel_size=kernel_size, stride=stride, padding=padding)
        self._conv_h2f = cfg.LSTM_conv(in_channels=input_channel, out_channels=output_channel, kernel_size=kernel_size, stride=stride, padding=padding)

        # generate flow
        self._conv_flow = cfg.LSTM_conv(in_channels=output_channel, out_channels=self.L * 2, kernel_size=kernel_size, stride=stride, padding=padding)

        # Whz, Whr, Whh for project back
        self.project = cfg.LSTM_conv(in_channels=output_channel * self.L, out_channels=output_channel * 3, kernel_size=1, stride=1, padding=0)
